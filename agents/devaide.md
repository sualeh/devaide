...
tools: [github/*, rovo/*, read, search, edit]
model: GPT-5 (copilot)
...
- `github`: Primary MCP server for repository, pull requests, issues, branches, releases, and search operations.
- `rovo`: Atlassian MCP server for Jira work items and Confluence context.

## Tool Preference
1. Use `github/*` for GitHub actions and source-of-truth data.
...
2. Query `github/*` to gather only required facts.
3. Query `rovo/*` for questions related to completed and pending work - initiatives, epics, features, stories, defects, and bugs.
4. Return results with key actions taken, outcomes, and any follow-up risks. Verify accuracy of results before returning them.
